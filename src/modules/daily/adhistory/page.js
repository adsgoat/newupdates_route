...
import DailyHistoryTables from "@/components/reportshistory/dailyhistorytables";
export default function AdTable({ account, ad_id, timezone, collection, userdetails, theme }) {
    ...
    const getCampaignData = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`/api/reportshistory/daily/ad?accountNumber=${account}&timezone=${timezone}&network=${collection}&AdId=${ad_id}`)
                    ...
                    return (
                        {
                            ...
                            ncpl: ncpl.toString(),
                            roi: roi,
                            cpc: eachData.cpc ? (eachData.cpc).toString() : 0,
                            cpclinkclicks: cpcLinkClicks ? cpcLinkClicks.toString() : 0,
                            ...
                            fbclicks: eachData.fbClicks ? (eachData.fbClicks).toString() : 0,
                            fblinkclicks: eachData.fbLinkClicks ? (eachData.fbLinkClicks).toString() : 0,
                            ...
                            adId: eachData.ad_id,
                            M_tq: parseFloat(eachData.M_tq || 0).toFixed(2),
                            D_tq: parseFloat(eachData.D_tq || 0).toFixed(2),
                        }
                    )
                })
                : [];
            console.log(filteredData)
            AdNameRef.current = filteredData[0].adname;
            setDataLoader(true);
    ...
    const handleCopy = () => {
        navigator.clipboard.writeText(AdNameRef.current).then(() => {
            setCopied(true);
    ...
    const handlePauseOk = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                adset_id: budgetObject?.adset_id,
                ad_id: budgetObject?.ad_id,
                accountNumber: String(account),
                name: AdNameRef.current,
                email: userdetails?.email,
                activity: payloadForActivity,
            };

            const response = await axios.post(`/api/reportshistory/status/ad`, payload);
            ...
            message.success(`Ad ${newStatus.toLowerCase()}`);
        } catch (e) {
            setConfirmLoading(false);
            // setIsCampaignModalVisibles(false);
    ...
    return (
        ...
        >
            ...
            {showTables && (
                <DailyHistoryTables theme={theme} chartData={chartData} />
            )}
                            ...
                            >
                                <Spin spinning={confirmLoading}>
                                    <div>
                                        ...
                                        <p style={{ color: theme === "dark" ? "white" : "black" }}>Ad ID: {ad_id}</p>
                                        <p style={{ color: theme === "dark" ? "white" : "black" }}>Ad Name: {AdNameRef.current}</p>
                                    </div>
                                </Spin>
                            </Modal>
                        </div>
