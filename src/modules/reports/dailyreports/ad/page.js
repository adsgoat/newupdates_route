...
export default function AdsetTable({ theme, userData, updatedRevenuePartner, updatedAccountsValue, updatedStartDate, updatedEndDate, updatedTime, userColumnStructure, campaignMap, adsetMap, commentsMap, Camapignlevelstatus, setCamapignlevelstatus, campaignComments, setCampaignComments, adLevelCreatives, setAdLevelCreatives, searchValue, gridRef, activeTab, activeTabForLiveReports, selectedAdsets, selectedCampaigns, showAdLevel, leafCampaigns, setLeafCampaigns, handleColumnMove, getMainMenuItems, customColumns, taxDetails, refreshTabs, refresh, userdetails }) {
    ...
    const memorizedAdData = useMemo(() => adData, [adData])
    ...
    const adDataRef = useRef([]);
    const lastAdFetch = useRef(null);
    const fbCron = useRef();
    ...
    const onBtExport = async () => {
        ...
        gridRef.current.api.forEachLeafNode(node => {
            ...
            if (d.campaignid && d.campaignname) {
                campaignMap[String(d.campaignid)] = d.campaignname;
            }
            if (d.adsetid && d.adsetname) {
                adsetMap[String(d.adsetid)] = d.adsetname;
            }
            if (d.adid && d.adname) {
                adMap[String(d.adid)] = d.adname;
            }
        });
        gridRef.current.api.exportDataAsCsv({
            processRowGroupCallback: (params) => {
                ...
                if (adMap[key]) return adMap[key];
                if (adsetMap[key]) return adsetMap[key];
                if (campaignMap[key]) return campaignMap[key];
            ...
            processCellCallback: (params) => {
                ...
                if (colId === "campaignid") return d.campaignname || params.value;
                if (colId === "adsetid") return d.adsetname || params.value;
                if (colId === "adid") return d.adname || params.value;
        ...
        try {
            ...
            console.error("Export activity failed", e);
        }
    }
    ...
    const handleBulkStatusSubmit = async () => {
        try {
            ...
            // Build current ad status map from Camapignlevelstatus
            const adStatusMap = new Map();

            (Camapignlevelstatus || []).forEach(campaign => {
                (campaign.adsets || []).forEach(adset => {
                    (adset.ads || []).forEach(ad => {
                        if (ad.ad_id) {
                            const key = `${campaign.accountNumber}_${ad.ad_id}`;

                            adStatusMap.set(
                                key,
                                String(ad.status || "ACTIVE").toUpperCase()
                            );
                        }
                    });
                });
            ...
            // Filter only ads that actually need the requested status
            const filteredItems = leafCampaigns
                .filter(item => {
                    const adKey = `${item.accountNumber}_${item.adid}`;

                    const currentStatus = adStatusMap.has(adKey) ? adStatusMap.get(adKey) : "ACTIVE";
                ...
                .map(item => ({
                    ...
                    activity: {
                        ...
                        updateDetail: `From ${bulkStatusAction === "ACTIVE" ? "Inactive" : "Active"
                            } to ${bulkStatusAction === "ACTIVE" ? "Active" : "Inactive"
                            }`,
                    }
                }));

            if (filteredItems.length === 0) {
                message.info("No ads need updating.");
                setIsBulkStatusModalVisible(false);
                return;
            }

            const resp = await apiClient.post("api/reports/status/ad/multiple", { items: filteredItems });
            ...
            if (resp.status === 200 || resp.status === 201) {
                ...
                const updatedCampaignLevelStatus = (
                    ...
                    adsets: (campaign.adsets || []).map(adset => ({
                        ...
                        ads: (adset.ads || []).map(ad => {
                            const update = filteredItems.find(item =>
                                String(item.campaign_id) ===
                                String(campaign.campaign_id) &&
                                String(item.accountNumber) ===
                                String(campaign.accountNumber) &&
                                String(item.adset_id) ===
                                String(adset.adset_id) &&
                                String(item.ad_id) ===
                                String(ad.ad_id)
                            );
                ...
                message.success(
                    `${filteredItems.length} ad(s) updated.`
                );
            }

        }
        catch (err) {
            console.error("Bulk ad status update error:", err);
            message.error("Bulk ad update failed");
        } finally {
            setIsBulkStatusModalVisible(false);
                    ...
                    leafCampaigns.forEach(item => {
                        const idx = arr.findIndex(
                            i =>
                                ...
                                String(i.ad_id) === String(item.adid) &&
                                i.level === 'ad'
                        );
    ...
    const handleBulkDeleteComments = async () => {
        try {
            ...
            const body = { accountNumber: leafCampaigns?.[0]?.accountNumber, level: 'ad', items };
            ...
            if (resp.status === 200 || resp.status === 201) {
                ...
                message.success({ content: `${leafCampaigns.length} comment(s) deleted successfully`, key: 'bulkDelete', duration: 3 });
    ...
    const handleBulkSmartPin = async (action) => {
        if (!leafCampaigns || leafCampaigns.length === 0) {
            message.warning("Select at least one ad");
            return;
        }

        // Convert adid → ad_id for backend
        ...
        message.loading(`${action === "pin" ? "Pinning" : "Unpinning"} selected ads...`, 0);
            ...
            data.forEach(item => {
                const idx = arr.findIndex(
                    i =>
                        ...
                        String(i.Account) === String(item.accountNumber) &&
                        String(i.adsetid) === String(item.adsetid) &&
                        String(i.ad_id) === String(item.ad_id) &&   // 👈 MUST MATCH ad_id
                        i.level === "ad"
                );
                if (idx >= 0) {
                    arr[idx] = { ...arr[idx], pinned: action === "pin", userPin: userdetails?.email, editedPinTime: timeStamp };
                } else {
                    arr.push({
                        campaignid: item.campaignid,
                        Account: item.accountNumber,
    ...
    const autoGroupColumnDef = {
        ...
        valueFormatter: (params) => {
            ...
            if (params.node?.group) {
                ...
                return firstChild?.adname ?? params.node.key;
            }
        ...
        cellRendererParams: {
            ...
            innerRenderer: (params) => {
                ...
                const adid = (() => {
                    if (params.node.group && params.node.childrenAfterGroup?.length > 0) {
                        return params.node.childrenAfterGroup[0]?.data?.adid ?? null;
                    }
                    return params.data?.adsetid ?? null;
                })();
                ...
                const handlePauseOk = async () => {
                    ...
                    try {
                        ...
                        const response = await apiClient.post(`/api/reports/status/ad/single`, payload);
                ...
                React.useEffect(() => {
                    setUniversalObject(campaignComments.find(d => d.level === "ad" && d.campaignid === campaignid && d.adsetid === adsetid && d.ad_id === adid && d.Account === rowAccount));
                    const rec = campaignComments.find(d => d.level === "ad" && d.campaignid === campaignid && d.adsetid === adsetid && d.ad_id === adid && d.Account === rowAccount)
                ...
                //     const matchedAd = arrayOfAdsets.find(a => String(a.adset_id) === String(adsetid))?.ads?.find(ad => String(ad.ad_id) === String(adid) );
                //     setSwitchOn(matchedAd ? matchedAd.status === "ACTIVE" : false);
                //     setConfirmLoading(false);
                // }, [Camapignlevelstatus, campaignid, adsetid, adid, rowAccount]);
                useEffect(() => {
                    const campaign = Camapignlevelstatus.find(s => String(s.campaign_id) === String(campaignid) && String(s.accountNumber) === String(rowAccount));

                    const matchedAd = campaign?.adsets?.find(a => String(a.adset_id) === String(adsetid))?.ads?.find(ad => String(ad.ad_id) === String(adid));

                    setSwitchOn(matchedAd?.status === "ACTIVE");
                    setConfirmLoading(false);
                }, [Camapignlevelstatus, campaignid, adsetid, adid, rowAccount]);
                ...
                const CreativeTooltip = () => {
                    if (!adLevelCreatives.length) {
                        return (
                            <div style={{ color: theme === "dark" ? "#e4e6eb" : "#050505" }}>
                                Loading creatives…
                            </div>
                        );
                    }
                    ...
                    return (
                        <div
                            onClick={(e) => {
                                e.stopPropagation();
                                setSelectedCampaignCreatives([matchedAd]);
                                setCreativeModalOpen(true);
                            }}
                        >
                            <CreativeMedia
                                url={matchedAd.image}
                                filetype={matchedAd.filetype}
                                small
                            />
                        </div>
                    );
                                ...
                                <Spin size="small" spinning={!Camapignlevelstatus.length} indicator={<SyncOutlined spin style={{ fontSize: 12, color: "#91c25f", marginLeft: 0 }} />}>
                                    ...
                                    >
                                        <PictureOutlined
                                            ...
                                            style={{
                                                cursor: hasCreative ? "pointer" : "not-allowed",
                                                color: hasCreative ? "#1890ff" : "#aaa",
                                                fontSize: 12,
                                                marginRight: '5px',
                                            }}
                                        />
                        ...
                        >
                            <Spin spinning={statusLoading} >
                                <div style={{ fontSize: 12 }}>
                                    <p>
                                        Are you sure you want to turn {nextChecked ? "ON" : "OFF"} this ad?
                                    </p>
                                    <p>Ad ID: {adid}</p>
                                    <p>Ad Name: {adName}</p>
                                </div>
    ...
    const creativesForCampaigns = async (adData, filtered) => {
        const uniqueCampaignIds = [
            ...new Set(
                adData.map(item => String(item.campaignid)).filter(Boolean)
            )
        ];
        ...
        const items = Object.entries(
            adData.reduce((acc, { accountNumber, campaignid }) => {
    ...
    const functionCall1 = async (creativesForCampaigns) => {
        try {
            setDataLoader(false);
            const collectionName = newtworkCollectionsForAds[updatedRevenuePartner];
            const adsets = selectedAdsets.length > 0 ? [...new Set(selectedAdsets.map(adset => adset.adsetid))] : [];
            const campaigns = selectedCampaigns.length > 0 ? [...new Set(selectedCampaigns.map(c => c.campaignid))] : [];
            ...
            const resp = await axios.post('/api/reports/live/ad', {
                dateStart: updatedStartDate,
                dateEnd: updatedEndDate,
                timezone: updatedTime,
                status: "Daily",
            ...
            adDataRef.current = (filteredData)
            let filtered = []
            if (searchValue) {
                ...
                if (normalizedInput) {
                    filtered = filtered.filter((each) => {
                        const adname = (each?.adname || "").trim().toLowerCase();
                        ...
                        const adId = (each?.adid || "").toString().trim().toLowerCase();
                        return (
                            adname.includes(normalizedInput) || campaignId.includes(normalizedInput) || accountNumber.includes(normalizedInput) || adsetId.includes(normalizedInput) || adId.includes(normalizedInput)
                        );
                    });
                }
                ...
                if (normalizedInside) {
                    filtered = filtered.filter((each) => {
                        ...
                        return (
                            ...
                            adname.includes(normalizedInside) ||
    ...
    const columnDefsObject = useMemo(() => {
        const allColumns = columnDefsObjectForAd({
            theme,
            userdetails,
            Camapignlevelstatus,
    ...
    useEffect(() => {
        if (activeTab !== "2" || activeTabForLiveReports !== "3") {
            return;
        }
        ...
        if (lastAdFetch.current === currentKey) {
            return;
        }

        lastAdFetch.current = currentKey;

        setLoading(true);
        setDataLoader(false);
        functionCall1(creativesForCampaigns);
    }, [activeTab, activeTabForLiveReports, updatedAccountsValue, updatedStartDate, updatedEndDate, updatedTime, selectedCampaigns, selectedAdsets, refreshTabs, refresh]);
    useEffect(() => {
        ...
        const adStatusMap = new Map(
            Camapignlevelstatus.flatMap(campaign =>
                (campaign.adsets || []).flatMap(adset =>
                    (adset.ads || []).map(ad => [String(ad.ad_id), ad.status?.toUpperCase()])
                )
            )
        );
        ...
        setAdData(
            (adDataRef.current || []).filter(each => {
                const adId = String(each?.adid || "").trim().toLowerCase();
                ...
                const adName = String(each?.adname || "").trim().toLowerCase();

                if (campaignStatusFilter !== "All" && adStatusMap.get(adId) !== status) return false;
                ...
                if (input && !campaignName.includes(input) && !adsetName.includes(input) && !adName.includes(input) &&
                    !campaignId.includes(input) && !account.includes(input) && !adId.includes(input) && !adsetId.includes(input)
                ) return false;

                if (inside && !campaignName.includes(inside) && !account.includes(inside) && !campaignId.includes(inside) &&
                    !adsetName.includes(inside) && !adName.includes(inside) && !adId.includes(inside) && !adsetId.includes(inside)
                ) return false;
    ...
    // useEffect(() => {
    //     let filtered = adDataRef.current || [];
    //     // console.log(adDataRef.current);
    ...
    const uniqueByAdId = (arr) => {
        const map = new Map();
        arr.forEach(obj => {
            if (!map.has(obj.adid)) {
                map.set(obj.adid, obj);
            }
        });
    ...
    const onSelectionChanged = (event) => {
        ...
        // setSelectedAd(selectedData);
        showAdLevel(uniqueByAdId(selectedData));
        const leafCampaignsData = selectedNodes
            ...
            .map(node => ({
                ...
                adid: node.data.adid,
                adName: node.data.adname,
                fbStatus: node.data.fbStatus,
                level: "ad",
            }))
            .filter(item => item.adid && item.adName);
        const uniqueLeafCampaigns = Array.from(
            new Map(
                leafCampaignsData.map(item => [`${item.adid}_${item.accountNumber}`, item])
            ).values()
        );
