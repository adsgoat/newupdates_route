...
export default function AdsetTable({ account, adset_id, timezone, collection, userdetails, reportType, theme }) {
    ...
    const updateColumnStructureToDefault = async () => {
        await handleColumnMove(columnStructure);
        setHistoryColumns(columnStructure);
    }
    ...
    const getCampaignData = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`/api/reportshistory/live/adset?accountNumber=${account}&timezone=${timezone}&network=${collection}&AdsetId=${adset_id}`)
