...
export default function AdTable({ account, ad_id, timezone, collection, userdetails, reportType, theme }) {
    ...
    const updateColumnStructureToDefault = async () => {
        const api = gridRef.current?.api;
        if (!api || !columnStructure?.length) return;

        const defaultOrder = [...columnStructure];

        // Update column definitions in the required order
        setHistoryColumns(defaultOrder);

        // Force AG Grid column order
        requestAnimationFrame(() => {
            api.applyColumnState({
                state: defaultOrder.map(colId => ({ colId })),
                applyOrder: true,
            });
        });

        await handleColumnMove(defaultOrder);
    };
    const getMainMenuItems = (params) => {
        ...
        if (resetColumnsItemIndex > -1) {
            ...
            defaultItems[resetColumnsItemIndex] = {
                ...
                action: updateColumnStructureToDefault,
    ...
    const getCampaignData = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`/api/reportshistory/live/ad?accountNumber=${account}&timezone=${timezone}&network=${collection}&AdId=${ad_id}`)
